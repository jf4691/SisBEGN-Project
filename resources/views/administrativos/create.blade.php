...
@section('titulo', 'SISBEGN | Crear Administrativo')

@section('encabezado', 'Crear Administrativo')
...
<li class="breadcrumb-item active">
  <a href="{{ route('administrativos.index') }}">Administrativo</a>
</li>
<li class="breadcrumb-item active">
...
<!-- Box Primary -->
    <div class="card card-primary">
      <div class="card-header color-template">
        <h3 class="card-title"  style="color: #ffffff">Registrar Administrativo</h3>
      </div>
      <!-- Formulario -->
      {!! Form::open(['route' => 'administrativos.store', 'autocomplete' => 'off', 'method' => 'POST', 'class' => 'form-horizontal']) !!}
        <div class="card-body my-card-body">

          <!-- Id de Usuario -->
          <div class="form-group row{{ $errors->has('user_id') ? ' has-error' : '' }}">
            {!! Form::label('user_id', 'Buscar Administrativo', ['class' => 'col-sm-4 col-form-label', 'style' => 'text-align:right']) !!}
            <div class="col-sm-5">
              <select name="user_id" class="form-control select2" id="user_id2" aria-placeholder="--Seleccione un Administrativo--">
        ...
        <!-- Cargo -->
          <div class="form-group row{{ $errors->has('cargo') ? ' has-error' : '' }}">
            ...
            <div class="col-sm-5">
              {!! Form::text('cargo', old('cargo'), ['class' => 'form-control', 'placeholder' => 'Cargo del administrativo', 'required']) !!}
                @if ($errors->has('cargo'))
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    $('#user_id2').select2().on("change", function(e) {
      // var obj = $("#admin_id2").select2("data");
